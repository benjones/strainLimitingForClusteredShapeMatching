...
dynamics~\cite{Mueller:2007:PBD,Bender:2013:PBM,Bender:2014:ASO,Macklin:2014:UPP}.  Recently, however,
Bargteil and Jones~\shortcite{Bargteil:2014:SLF} incorporated strain-limiting into clustered shape matching
and pointed out several advantages of shape matching over position-based dynamics.  In follow-up work, Jones
...
\end{equation}
where $\B{g}_{i,c}$ is the goal position for particle $p_i$ in cluster $c$.  For details
details see the paper by Jones and colleagues~\shortcite{Jones:2015:CAC}.
...
disallow fracture for a cluster that has fractured until $\B{V}_{\max} < \tau_c$.  The second, more flexible,
...
Though particles decrease in mass when the split during fracture, we do not modify the size of particles in our renderings.  In this work, we focus on creating compelling motion, not production quality visualization.  Generating compelling, real-time renderings from particle data is an open problem that we do not address in this paper.
...
One limitation of our current implementation is that the computational cost is somewhat unpredictable; many fractures occurring at once

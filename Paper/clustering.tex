...
We then use the weights to divide the particles mass among the clusters, resulting in a very even distribution of mass over an 
...
colleagues~\shortcite{Mueller:2005:MDB}, {\em shape matching} is a
technique for animating deformable bodies modeled as volumetric
...
movements.  Even though the approach lacks a well-developed mathematical
underpinning, \Mueller et al.~illustrate many
advantages to the technique that make it suitable for interactive
graphics and in particular video games.  One of their key idea is 
...
The second algorithm uses $k$-means to determine cluster centers; given $k = |\mathcal{C}|$ 
...
the cluster centers given by $k$-means in the $\mathrm{k-means}$ algorithm; or the cluster centers given by our new algorithm.
Note that with $\mathrm{random}$ and $\mathrm{k-means}$ the algorithmic cluster centers and the physical cluster center of mass will

...
record the corresponding right singular vector $\B{V}_{\max}$.  At the end of the timestep, we iteratively remove
...
The colorings indicate which cluster center a particle is closest to.

Twisting bar with/without fracture.  The bar keeps some shape after the constraints are removed

Swiss cheese and heart: complex geometry affects where the cracks form and how they propagate

Armadillo/sheet: not sure what to say for them.  Interaction with projectiles I guess?
...
Currently we use an off-the-shelf particle skinning tool~\cite{Bhattacharya:2011:ALM} to skin the simulation particles.

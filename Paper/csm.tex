...
$\Sigma_{\max} > \tau_c$ before fracturing and also recompute $\B{V}_{\max}$.  \adam{Finally, we delete
...
plastic deformation, which allows the bar to maintain its twisted shape, then we enable fracture.  \ben{This twisting deformation causes the bar to fracture violently, spraying small pieces away from the object.}
\ben{In our third example a projectile is fired through a thin sheet of material with various material properties(see~\fref{fig:projectile}).  The weak, brittle material 
